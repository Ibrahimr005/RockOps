...
psql -U postgres -d oretechDB -c "DELETE FROM flyway_schema_history WHERE version = '20250104';"

# Verify it's gone
Write-Host "`nVerifying deletion..." -ForegroundColor Green
psql -U postgres -d oretechDB -c "SELECT version, description, success FROM flyway_schema_history ORDER BY installed_rank DESC LIMIT 5;"
