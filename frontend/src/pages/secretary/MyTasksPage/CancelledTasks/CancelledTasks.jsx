...
const ROLE_CLASS = {
    ...
    SECRETARY:          'can-role--secretary',
    WAREHOUSE_MANAGER:  'can-role--manager',
    WAREHOUSE_EMPLOYEE: 'can-role--employee',
};
...
const ExpandableDescription = ({ text }) => {
    ...
    return (
        <span className="can-description-cell">
            ...
            <button className="can-read-more-btn" onClick={(e) => { e.stopPropagation(); setExpanded(p => !p); }}>
                {expanded ? ' Less' : ' More'}
            </button>
        </span>
    );
...
const CancelledTasks = ({ tasks, loading, onDelete, showAssignee = false, showDelete = false }) => {
    ...
    const assigneeColumns = showAssignee ? [
        {
            ...
            render: (row) => {
                ...
                if (!role) return '-';
                return <span className={`can-role-badge ${ROLE_CLASS[role] || ''}`}>{ROLE_LABELS[role] || role}</span>;
            }
        },
        {
            id: 'assignedTo', header: 'ASSIGNED TO', accessor: 'assignedTo', sortable: true, minWidth: '160px',
            render: (row) => row.assignedTo
                ? <span className="can-assignee">{row.assignedTo.firstName} {row.assignedTo.lastName}</span>
                : '-'
        },
    ] : [];
    ...
    const deleteColumn = showDelete ? [{
        ...
        render: (row) => (
            <button className="can-action-btn can-action-btn--delete" onClick={(e) => openDialog(row.id, e)} title="Delete">
                <FaTrash />
            </button>
        )
    }] : [];
    ...
    const columns = [
        ...
        ...assigneeColumns,
        {
            ...
            render: (row) => {
                ...
                return (
                    <span className={`can-due-date ${isOverdue ? 'can-due-date--overdue' : ''}`}>
        ...
        {
            id: 'updatedAt', header: 'CANCELLED AT', accessor: 'updatedAt', sortable: true, minWidth: '160px',
